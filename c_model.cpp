...
// c_model.cpp is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.

// c_model.cpp is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.

// You should have received a copy of the GNU General Public License
// along with c_model.cpp.  If not, see <http://www.gnu.org/licenses/>. 
...
{ 
  ...
  double nyr = ny * _v[2];
  C(0,2) = -mxgr - myyv + nyr; // c02
  C(1,2) = -mygr + mxxu;       // c12
  C(2,0) = -C(0,2);           // c20
  C(2,1) = -C(1,2);           // c21
  
  // initializing force vector T
  Eigen::Vector3d T(f[0], f[1], f[2]);
  
  // Calculating next velocity
  // Mv'+(C+Dl+Dq)v=T
  // v'=M^-1 * (T-(C+Dl+Dq)v)
  // v=v+v'dt
  Eigen::Vector3d V(_v[0], _v[1], _v[2]);
  Eigen::Vector3d Vnext;
  Eigen::Vector3d R1 = (C + Dl + Dq) * V;
